...
export class GrnReprintComponent {
  // displayedColumns = [
  //   'id',
  //   'createdDate',
  //   'createdBy',
  ...
  // setBreadCrumb() {
  //   this.items = this.translate.instant('GRN_REPRINT.BREAD_CRUMB');
  ...
  getData() // Featch data
  {
    ...
    this.isLoading = true;
    this.dataService.grnList(sd, ed)
      .subscribe(
        {
          next: (response: any) => {
  ...
  //     this.commonService.alertMessage(alert, notice);
  //   } else {
  ...
  getPrint(data: any) {
    this.isLoading = true;
    this.dataService.getSelectedGrnDetails(data.id)
      .subscribe(
        {
          next: (response: any) => {
            if (response.status == 200) {
              PrintModule.printGrn(data, response.result);
            }
            else (this.featchError('Featch Error'))
          },
          error: () => this.featchError('connection error try again later'),
