...
export class VolumeTypeComponent {
  ...
  constructor(
    private dataService: VolumeTypeService,
    ...
    this.dataForm = this.formBuilder.group({
      ...
      name: new FormControl('', Validators.compose([Validators.required, Validators.pattern('^[a-zA-Z]+$'), Validators.maxLength(30)]))
    });
  }
  ...
  setBreadCrumb() {
    this.items = this.translate.instant('VOLUME_TYPE.BREAD_CRUMB');
      ...
      .subscribe(
        {
          next: (response: any) => {
            ...
            else (this.transactionError('VOLUME_TYPE.' + response.errDetail))
          },
  ...
  print() {
    const col = ["Volume Type ID", "Volume Type", "Created Date", "Created By"];
    const rows = [];
    ...
    new PrintModule().commonPrint(col, rows, 'Volume Type');
  }
